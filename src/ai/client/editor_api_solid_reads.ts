...
import { collectGreyBoxHierarchy } from './grey_box_hierarchy_payload.js';
...
export class EditorApiSolidReads {
  ...
  getEditorContext(): McpToolResult {
    const data: EditorContextDto = {
      ...
      greyBoxCount: GreyBoxRegistry.collectUnder(this.host.worldObject).filter((object) => !isGreyBoxGroup(object))
        .length,
  ...
  getSceneHierarchy(): McpToolResult {
    ...
    return okResult('Scene hierarchy', {
      children,
      greyBoxes: collectGreyBoxHierarchy(this.host.worldObject),
    });
  }
